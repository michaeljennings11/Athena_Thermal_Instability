...
HydroDiffusion::HydroDiffusion(Hydro *phyd, ParameterInput *pin) {
  pmy_hydro_ = phyd; //hydro pointer
  pmb_ = pmy_hydro_->pmy_block; //meshblock pointer
  pco_ = pmb_->pcoord; //coordinate pointer
  hydro_diffusion_defined = false; //hydro diffusion flag off

  int ncells1 = pmb_->block_size.nx1 + 2*(NGHOST); //add each end ghost zones to total 1D block size
  int ncells2 = 1, ncells3 = 1;
  if (pmb_->block_size.nx2 > 1) ncells2 = pmb_->block_size.nx2 + 2*(NGHOST); //add each end ghost zones to total 2D block size
  if (pmb_->block_size.nx3 > 1) ncells3 = pmb_->block_size.nx3 + 2*(NGHOST); //add each end ghost zones to total 3D block size
  ...
  if (nu_iso > 0.0 || nu_aniso  > 0.0) {
    hydro_diffusion_defined = true; //hydro diffusion flag on
    ...
    // If anisotropic, check for mirror and firehose limits
      if (nu_aniso > 0.0) {
        ...
        // It is generally more numerically and physically reliable to use the CGL EOS
        //std::cout << "### WARNING in HydroDiffusion::HydroDiffusion" << std::endl
        //<< "For most problems, it is recommended to use CGL with collisions "
        //<< "rather than anisotropic viscosity" << std::endl;
      }
    }
  ...
  if (NON_BAROTROPIC_EOS) {
    kappa_iso  = (mbar/kb)*pin->GetOrAddReal("problem","kappa_iso",0.0); // iso thermal conduction
    kappa_aniso  = (mbar/kb)*pin->GetOrAddReal("problem","kappa_aniso",0.0); // aniso conduction
...
const AthenaArray<Real> &bcc) {
  ...
  if (nu_aniso > 0.0) ViscousFlux_aniso(prim, cons, visflx, pf->b, pf->bcc);

  if (kappa_iso > 0.0 || kappa_aniso > 0.0) ClearHydroFlux(cndflx);
  if (kappa_iso > 0.0) ThermalFlux_iso(prim, cons, cndflx);
  if (kappa_aniso > 0.0) ThermalFlux_aniso(prim, cons, pf->b, pf->bcc, cndflx);
      ...
      if ((nu_iso > 0.0) || (nu_aniso > 0.0)) {
        for (int i=is; i<=ie; ++i)
          dt_vis = std::min(dt_vis, static_cast<Real>(SQR(len(i))
                                     *fac*pmb_->phydro->w(IDN,k,j,i)/(nu_t(i)+TINY_NUMBER)));
      }
      if ((kappa_iso > 0.0) || (kappa_aniso > 0.0)) {
        for (int i=is; i<=ie; ++i)
