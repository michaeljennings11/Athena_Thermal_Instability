...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real brad = pin->GetReal("problem","brad");
  Real amp = pin->GetReal("problem","amp");
  Real vflow = pin->GetReal("problem","vflow");
  Real drat = pin->GetOrAddReal("problem","drat",1.0);
  Real drad = pin->GetOrAddReal("problem","drad",0.25);
  ...
  Real ang_2,cos_a2,sin_a2,lambda, rad;
...
// Use vector potential to initialize field loop
  ...
  for (int i=is; i<=ie; i++) {
     ...
     if ((SQR(pcoord->x1v(i)) + SQR(pcoord->x2v(j)-x2size/4.0) + SQR(pcoord->x3v(k))) < drad*drad) {
       phydro->u(IDN,k,j,i) = 1.0/drat;
       phydro->u(IM1,k,j,i) = 0.0;//phydro->u(IDN,k,j,i)*vflow*x1size/diag;
       phydro->u(IM2,k,j,i) = 0.0;//phydro->u(IDN,k,j,i)*vflow*x2size/diag;
       phydro->u(IM3,k,j,i) = 0.0;//phydro->u(IDN,k,j,i)*vflow*x3size/diag;
     }
