...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  if (shk_dir == 1) {
    im1 = IM1; im2 = IM2; im3 = IM3; // array indices found in athena.hpp
    ib1 = IB1; ib2 = IB2; ib3 = IB3; // IM* = momentum
  } else if (shk_dir == 2) {
  ...
  Real err[NHYDRO+NFIELD]; //NHYDRO = 5, NFIELD = 3 found in defs.hpp
  for (int i=0; i<(NHYDRO+NFIELD); ++i) err[i]=0.0;

  // Errors in RJ2a test (Dai & Woodward 1994 Tables Ia and Ib)
  if (MAGNETIC_FIELDS_ENABLED) { //MAGNETIC_FIELDS_ENABLED default = 0 found in defs.hpp
